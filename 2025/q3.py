...
def part2():           # => 226
    ...
    # I'll convert the list to a set in order to remove the duplicates.
    # Then, if I sort the list in ascending order, the first 20 values should be the smallest total size.
...
def part3(data):           # => 2229
    ...
    # In order to find the minimum number of sets, I just need to find the maximum number of duplicates in the list...
    # since there can't be duplicates in the crates. For example, if there are three 5's, then there has to be at least
    # three different sets because each 5 has to be in its own set.

    min_sets = max(x.count(n) for n in set(x))
